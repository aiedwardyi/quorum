                  ...
                  <div className="p-5 rounded-xl bg-secondary/30 border border-border/60">
                    <div className="flex items-center justify-between gap-4">
                      <div>
                        ...
                        <div className="flex items-center gap-2"><span className="text-2xl font-semibold tracking-tight text-foreground">{(tier === "anonymous" || tier === "free" ? (freeDebatesRemaining ?? 0) : (balance ?? 0)) === 999 ? (locale === "ko" ? "무제한" : "Unlimited") : (tier === "anonymous" || tier === "free" ? (freeDebatesRemaining ?? 0) : (balance ?? 0).toLocaleString())}</span><Star className="w-4 h-4 text-yellow-400 fill-yellow-400" /></div>
                      </div>
