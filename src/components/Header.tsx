...
export default function ChatHeader({
  ...
  const confirmIfDebating = useCallback(
    (action: () => void) => {
      if (isDebating) {
    ...
    <>
      <header className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 px-4 sm:px-6 py-3 border-b border-border bg-background/80 backdrop-blur-md sticky top-0 z-50">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <button
            onClick={() =>
              confirmIfDebating(() => (onNewDebate ? onNewDebate() : router.push("/")))
            }
          ...
          <div className="flex items-center gap-1.5 sm:gap-4">
            ...
            <div
              className={cn(
                "relative shrink-0 group hidden sm:block",
            ...
            >
              <motion.button
                whileTap={{ scale: 0.95 }}
                onClick={() => {
                  setShowRoundsDropdown(!showRoundsDropdown)
                  setShowLengthDropdown(false)
                  setShowUserMenu(false)
                }}
              ...
              >
                ...
                <span className="text-[11px] sm:text-xs font-medium text-zinc-600 dark:text-zinc-400">
                  {isDebating
                    ...
                    : `${maxRounds} ${maxRounds === 1 ? t.round : t.rounds}`}
                </span>
        ...
        <div className="flex items-center gap-2 sm:gap-4 ml-auto">
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <motion.button
              ...
              className={cn(
                "w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-all group",
                theme === "lovelace" && "hover:ring-[1.5px] hover:ring-[#eb6f92]/60",
                theme === "tokyonight" && "hover:ring-[1.5px] hover:ring-[#7aa2f7]/40",
                theme === "gruvbox" && "hover:ring-[1.5px] hover:ring-[#fe8019]/50",
                theme === "catppuccin" && "hover:ring-[1.5px] hover:ring-[#cba6f7]/50",
